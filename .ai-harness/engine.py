...
def ensure_dirs() -> None:
    RUNS.mkdir(parents=True, exist_ok=True); MEMORY.mkdir(parents=True, exist_ok=True)


def compact(text: str, limit: int) -> str:
    text = text.strip()
    if len(text) <= limit: return text
    head = max(1, int(limit * .72)); tail = max(1, limit - head - 60)
    return text[:head] + "\n... [compacted] ...\n" + text[-tail:]
...
def write_json_atomic(path: Path, value: object) -> None: write_text_atomic(path, json.dumps(value, indent=2, ensure_ascii=False) + "\n")
...
def run_capture(command: list[str], cwd: Path = ROOT, timeout: int = 20) -> tuple[int,str]:
    try: result=subprocess.run(command,cwd=cwd,text=True,capture_output=True,check=False,timeout=timeout)
    except subprocess.TimeoutExpired as exc: return 124,f"TIMEOUT: {exc}"
    except OSError as exc: return 127,f"ERROR: {exc}"
...
def git_state() -> str:
    ...
    for command in (["git","status","--short"],["git","branch","--show-current"],["git","rev-parse","HEAD"]):
        code,output=run_capture(command); parts.append(f"$ {shlex.join(command)}\n{output if code==0 else 'ERROR: '+output}")
    return "\n".join(parts)
...
def diff_summary() -> str:
    ...
    for command in (["git","diff","--stat"],["git","diff","--check"],["git","diff","--name-only"]):
        _,output=run_capture(command); parts.append(f"$ {shlex.join(command)}\n{output}")
    return "\n".join(parts)
...
def diff_check() -> tuple[bool,str]:
    code,output=run_capture(["git","diff","--check"]); return code==0,output


def make_run_dir() -> Path:
    ensure_dirs(); stamp=dt.datetime.now(dt.UTC).strftime("%Y%m%dT%H%M%SZ"); path=RUNS/stamp; suffix=1
    while path.exists(): path=RUNS/f"{stamp}-{suffix}"; suffix+=1
...
def repository_map(limit:int=500)->str:
    ...
    if code!=0:return "# Repository Map\nUnavailable: git ls-files failed."
    files=[x for x in output.splitlines() if x][:limit]; return "# Repository Map\n\n"+"\n".join(f"- {x}" for x in files)
...
def profile_repository()->dict[str,Any]:
    ...
    if code!=0:return {"profile_error":compact(output,500)}
    try:value=json.loads(output)
    except json.JSONDecodeError:return {"profile_error":"project_profile.py returned invalid JSON"}
...
def relevant_memory(task:str,budget:int=900)->str:
    items=read_jsonl(MEMORY/"patterns.jsonl")+read_jsonl(MEMORY/"observations.jsonl"); words=normalize_words(task); ranked=[]
    for item in items:
        if item.get("status") not in (None,"trusted","candidate"):continue
        text=" ".join(str(item.get(k,"")) for k in ("pattern","lesson","scope","task","tags")); overlap=len(words&normalize_words(text)); confidence=float(item.get("confidence",.5))
        if overlap or confidence>=.9:ranked.append((overlap*2+confidence,item))
    ranked.sort(key=lambda x:x[0],reverse=True); selected=[]; used=0
    for _,item in ranked[:12]:
        encoded=json.dumps(item,ensure_ascii=False)
        if used+len(encoded)>budget:break
        selected.append(encoded); used+=len(encoded)
    return "\n".join(selected) or "No relevant learned patterns."


def load_principles(limit:int=7000)->str:
    if not PRINCIPLES.exists():return "Use language-neutral engineering judgment focused on correctness, simplicity, maintainability, security, compatibility, testing, failure awareness and evidence."
    return compact(PRINCIPLES.read_text(encoding="utf-8"),limit)


def heuristic_route(task:str)->dict[str,Any]:
    text=task.lower(); mode="implement"; risk="low"; uncertainty="known"; capabilities=[]
    explicit_poc=any(t in text for t in ("poc","proof of concept","feasibility","spike","prototype","experiment")); explicit_review=any(t in text for t in ("review","assess","audit")); explicit_research=any(t in text for t in ("research","investigate options","which library","which framework")); debug_signals=("why","diagnose","intermittent","root cause","failing","hang","error","regression","broken","suspected authorization bypass","occasionally duplicates","became 10x slower")
    if explicit_poc:mode="poc";capabilities.extend(("research","poc"));uncertainty="unknown"
    elif explicit_review:mode="review"
    elif explicit_research:mode="research";capabilities.append("research");uncertainty="moderate"
    elif any(t in text for t in debug_signals) and "do not change anything" not in text and "without changing behavior" not in text and "already agreed constant" not in text:mode="debug";uncertainty="moderate"
    if mode=="implement" and ("flexible approval workflow" in text or "configurable rules" in text):mode="grill";capabilities.append("grill")
    if any(t in text for t in ("security","authentication","authorization","production","migration","breaking","performance","scale","release","approved spec","engineering standards")):capabilities.append("grill");risk="high"
    if mode=="debug" and "production" in text:capabilities.append("grill");risk="high"
    if mode=="review" and any(t in text for t in ("security","authentication","approved spec")):capabilities.append("grill");risk="high"
...
def normalize_route(value:dict[str,Any])->dict[str,Any]:
    route=dict(value); route["mode"]=route.get("mode") if route.get("mode") in MODES else "implement"; route["risk"]=route.get("risk") if route.get("risk") in RISKS else "low"; route["uncertainty"]=route.get("uncertainty") if route.get("uncertainty") in UNCERTAINTIES else "known"; route["capabilities"]=list(dict.fromkeys(c for c in route.get("capabilities",[]) if c in CAPABILITIES)); route["scope"]=str(route.get("scope","repository")); route["confidence"]=max(0.0,min(1.0,float(route.get("confidence",.5)))); route["reason"]=compact(str(route.get("reason","")),300); return route


def parse_route(output:str)->dict[str,Any]|None:
    for line in output.splitlines():
        if "ROUTE_JSON:" not in line:continue
        try:value=json.loads(line.split("ROUTE_JSON:",1)[1].strip())
...
def provider_command(provider:dict[str,Any],prompt_file:Path,phase:str,run_dir:Path)->tuple[list[str],Path]:
    replacements={"{prompt_file}":str(prompt_file),"{workspace}":str(ROOT),"{phase}":phase,"{run_dir}":str(run_dir),"{python}":sys.executable}; command=[replacements.get(v,v) for v in provider["command"]]; working=replacements.get(provider.get("working_directory","{workspace}"),provider.get("working_directory",str(ROOT))); return command,Path(working)
...
def invoke(provider:dict[str,Any],prompt_file:Path,phase:str,run_dir:Path,timeout:int,dry_run:bool,logger)->tuple[int,str,float]:
    command,cwd=provider_command(provider,prompt_file,phase,run_dir); output_file=run_dir/f"{phase}.output.md"; started=time.monotonic(); emit_event(run_dir,"provider.start",phase=phase,provider=provider.get("name"))
    if dry_run:
        output="DRY RUN\n$ "+shlex.join(command); write_text_atomic(output_file,output+"\n"); return 0,output,0.0
    try:result=subprocess.run(command,cwd=cwd,env=os.environ.copy(),text=True,capture_output=True,check=False,timeout=timeout)
    except subprocess.TimeoutExpired as exc:output=f"TIMEOUT after {timeout}s\n{exc}";code=124
    except OSError as exc:output=f"ERROR: {exc}";code=127
    else:output=(result.stdout or "")+(("\n"+result.stderr) if result.stderr else "");code=result.returncode
    write_text_atomic(output_file,output);duration=time.monotonic()-started;emit_event(run_dir,"provider.finish",phase=phase,provider=provider.get("name"),exit_code=code,duration_seconds=round(duration,3)); logger.info("phase=%s provider=%s exit=%s duration=%.3fs",phase,provider.get("name"),code,duration); return code,output,duration
...
def build_prompt(phase:str,task:str,source:str,jira:str|None,route:dict[str,Any],repo_map:str,memory:str,profile:dict[str,Any],history:str)->str:
    phase_path=PROMPTS/f"{phase}.md"; phase_rules=phase_path.read_text(encoding="utf-8") if phase_path.exists() else ""
...
def save_control_checkpoint(run_dir:Path,manifest:dict[str,Any],phase:str,index:int,total:int,output:str)->dict[str,Any]:
    ...
    write_json_atomic(run_dir/f"checkpoint-{index:02d}-{phase}.json",record); write_json_atomic(run_dir/"execution-checkpoint.json",record); emit_event(run_dir,"execution.checkpoint",phase=phase,index=index,next_action=record["next"]); return record
...
def route_with_provider(provider:dict[str,Any],task:str,source:str,jira:str|None,repo_map:str,memory:str,run_dir:Path,config:dict[str,Any],dry_run:bool,logger)->dict[str,Any]:
    prompt=f"""Route this engineering task without changing files. Return exactly one line beginning with ROUTE_JSON: followed by JSON containing mode, capabilities, risk, uncertainty, scope, reason, confidence. Use the minimum safe route.\n\nTask source: {source}\nJira: {jira or 'none'}\nTask: {task}\n\nRepository map:\n{compact(repo_map,int(config['router']['context_budget']))}\n\nRelevant memory:\n{compact(memory,int(config['router']['memory_budget']))}\n"""; prompt_file=run_dir/"route.prompt.md"; write_text_atomic(prompt_file,prompt); code,output,duration=invoke(provider,prompt_file,"route",run_dir,int(config["execution"]["provider_timeout_seconds"]),dry_run,logger); route=parse_route(output) if code==0 else None; route=route or heuristic_route(task); route["router_duration_seconds"]=round(duration,3); route["router_source"]="provider" if code==0 and parse_route(output) else "heuristic"; return route


def phases_for(route:dict[str,Any],workflow:str|None,config:dict[str,Any])->list[str]:
    if workflow:
        workflows=config.get("workflows",{});selected=workflows.get(workflow)
        if not isinstance(selected,dict) or not isinstance(selected.get("phases"),list):raise ConfigurationError(f"Unknown workflow: {workflow}")
        return [str(p) for p in selected["phases"]]
    phases=["context"];mode=route["mode"];caps=set(route["capabilities"])
    ...
    phases.append("learn");return list(dict.fromkeys(phases))
...
def run_validation(config:dict[str,Any],run_dir:Path):
    ...
    results=[];log=run_dir/"validation.log"
    with log.open("w",encoding="utf-8") as stream:
        for command in commands[:5]:
            stream.write(f"$ {shlex.join(command)}\n");start=time.monotonic()
            try:result=subprocess.run(command,cwd=ROOT,stdout=stream,stderr=subprocess.STDOUT,text=True,check=False,timeout=int(config["execution"]["validation_timeout_seconds"]));code=result.returncode;status="passed" if code==0 else "failed"
...
def learn(run_dir:Path,task:str,route:dict[str,Any],manifest:dict[str,Any])->None:
    review_path=run_dir/"review.output.md"; review=review_path.read_text(encoding="utf-8") if review_path.exists() else ""; validation_ok=bool(manifest.get("validation",{}).get("passed",True)); lessons=[]
    for line in review.splitlines():
        clean=line.strip(" -#")
        if clean and any(word in clean.lower() for word in ("lesson","recommend","avoid","prefer","root cause")):lessons.append(compact(clean,500))
    ...
    for lesson_text in lessons[:5]:append_jsonl(MEMORY/"patterns.jsonl",{"id":hashlib.sha256(lesson_text.encode()).hexdigest()[:12],"created_at":now_iso(),"pattern":lesson_text,"scope":route.get("scope","repository"),"confidence":.6 if validation_ok else .35,"status":"candidate","success":validation_ok})


def groom_memory(config:dict[str,Any])->dict[str,Any]:
    patterns_path=MEMORY/"patterns.jsonl"; observations=read_jsonl(MEMORY/"observations.jsonl"); patterns=read_jsonl(patterns_path); min_obs=int(config["learning"]["min_observations_for_promotion"]); floor=float(config["learning"]["min_success_rate_for_promotion"]); max_items=int(config["learning"]["max_memory_items"]); outcomes={}
    ...
    write_text_atomic(patterns_path,"".join(json.dumps(item,ensure_ascii=False)+"\n" for item in chosen));return {"patterns":len(chosen),"trusted":sum(1 for item in chosen if item.get("status")=="trusted")}
...
def evaluate()->dict[str,Any]:
    ...
    passed=sum(1 for item in results if item["passed"]);return {"cases":len(results),"passed":passed,"failed":len(results)-passed,"results":results}


def run_task(args:argparse.Namespace,config:dict[str,Any],logger)->int:
    providers=config.get("providers",{});provider_name=args.agent or config.get("harness",{}).get("default_provider")
    ...
    if args.resume:
        ...
        manifest={"version":6,"run_id":run_dir.name,"started_at":now_iso(),"provider":provider_name,"task":task,"source":source,"jira":jira,"workflow":args.workflow or "adaptive","route":route,"phases":phases,"task_chunks":chunks,"completed_phases":[],"retries":0,"repository_profile":profile,"initial_git":git_state(),"protected_paths":config.get("scope",{}).get("protected_paths",[]),"scope_paths":config.get("scope",{}).get("allowed_paths",[])}
    ...
    for index,phase in enumerate(phases):
        ...
        if phase=="context":manifest.setdefault("completed_phases",[]).append(phase);save_checkpoint(run_dir,manifest,phase,"completed",next_phase);save_control_checkpoint(run_dir,manifest,phase,index+1,len(phases),output);continue
        if phase=="validate":
        ...
        if phase=="learn":
            learn(run_dir,task,route,manifest);manifest.setdefault("completed_phases",[]).append(phase);write_json_atomic(run_dir/"manifest.json",manifest);save_checkpoint(run_dir,manifest,phase,"completed",next_phase);save_control_checkpoint(run_dir,manifest,phase,index+1,len(phases),"learning completed");continue
        prompt_file=run_dir/f"{phase}.prompt.md";memory=relevant_memory(task,int(config["router"]["memory_budget"]));context=(run_dir/"repository-map.md").read_text(encoding="utf-8");prompt=build_prompt(phase,task,source,jira,route,context,memory,profile,"\n\n".join(history));write_text_atomic(prompt_file,prompt);succeeded=False;max_attempts=int(config["execution"]["max_phase_retries"])+1
        for attempt in range(max_attempts):
            if attempt:manifest["retries"]+=1;prompt=build_prompt(phase,task,source,jira,route,context,memory,profile,"\n\n".join(history)+"\nPrevious attempt failed. Gather new evidence and change the approach.");write_text_atomic(prompt_file,prompt)
        ...
        if not succeeded:manifest["status"]="failed";write_json_atomic(run_dir/"manifest.json",manifest);save_checkpoint(run_dir,manifest,phase,"failed",phase);return 1
...
def main()->int:
    parser=command_line();args=parser.parse_args();config=load_config();logger=configure_logging(None,str(config.get("observability",{}).get("log_level","INFO")));run_dir=None
    try:
        ...
        if args.resume:run_dir=Path(args.resume).resolve();logger=configure_logging(run_dir,str(config.get("observability",{}).get("log_level","INFO")));return run_task(args,config,logger)
        run_dir=make_run_dir();logger=configure_logging(run_dir,str(config.get("observability",{}).get("log_level","INFO")));emit_event(run_dir,"run.start",provider=args.agent,workflow=args.workflow);return run_task(args,config,logger)
    except HarnessError as exc:
        if run_dir:emit_event(run_dir,"run.error",**exception_summary(exc))
        logger.error("harness error: %s",exc);return 2
    except Exception as exc:
        if run_dir:emit_event(run_dir,"run.crash",**exception_summary(exc))
