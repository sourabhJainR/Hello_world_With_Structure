#!/usr/bin/env python3
"""Detect repository conventions and expose exact constructs and compatibility constraints."""
...
def _naming_styles(files: list[Path]) -> dict[str, int]:
    ...
    for path in files:
        stem = path.stem
        if re.fullmatch(r"[A-Z][A-Za-z0-9]*", stem): counts["PascalCase"] += 1
        elif re.fullmatch(r"[a-z][A-Za-z0-9]*", stem) and any(ch.isupper() for ch in stem): counts["camelCase"] += 1
        elif re.fullmatch(r"[a-z0-9]+(?:_[a-z0-9]+)+", stem): counts["snake_case"] += 1
        elif re.fullmatch(r"[a-z0-9]+(?:-[a-z0-9]+)+", stem): counts["kebab-case"] += 1
        elif re.fullmatch(r"[a-z0-9]+(?:\.[a-z0-9]+)+", stem): counts["dot.case"] += 1
    return dict(counts)


def _type_naming(files: list[Path]) -> dict[str, int]:
    counts = Counter(); pattern = re.compile(r"\b(?:class|interface|struct|record|enum)\s+([A-Za-z_][A-Za-z0-9_]*)")
    for path in files:
        try: text = path.read_text(encoding="utf-8", errors="ignore")
        except OSError: continue
        for name in pattern.findall(text):
            if re.fullmatch(r"[A-Z][A-Za-z0-9]*", name): counts["PascalCase"] += 1
            elif re.fullmatch(r"[a-z][A-Za-z0-9]*", name): counts["camelCase"] += 1
            elif "_" in name: counts["snake_case"] += 1
    return dict(counts)
...
def build_profile() -> dict[str, Any]:
    root_files = {p.name for p in ROOT.iterdir() if p.is_file()}; source_files = _source_files(); languages: list[str] = []
    if "pyproject.toml" in root_files or "requirements.txt" in root_files or any(p.suffix == ".py" for p in source_files): languages.append("python")
    if any(p.suffix in {".csproj", ".sln"} for p in source_files): languages.append("csharp")
    if "package.json" in root_files or any(p.suffix in {".js", ".jsx", ".ts", ".tsx"} for p in source_files): languages.append("javascript-typescript")
    if "go.mod" in root_files: languages.append("go")
    if "Cargo.toml" in root_files: languages.append("rust")
    if "pom.xml" in root_files or "build.gradle" in root_files or "build.gradle.kts" in root_files: languages.append("java-kotlin")
    ...
    file_naming = _naming_styles(source_files); type_naming = _type_naming(source_files); construct_index = build_index(ROOT); compatibility = build_compatibility_profile(ROOT)
    return {
        ...
        "naming": {"file_styles": file_naming, "dominant_file_style": max(file_naming, key=file_naming.get) if file_naming else "repository-defined", "type_styles": type_naming, "dominant_type_style": max(type_naming, key=type_naming.get) if type_naming else "language-default", "test_locations": _test_locations(source_files)},
        ...
        "compatibility": {**compatibility, "instructions": compatibility_instructions(compatibility)},
        "construct_traceability": {"enabled": True, "schema_version": construct_index["schema_version"], "files_scanned": construct_index["files_scanned"], "construct_count": construct_index["construct_count"], "reference_format": "[construct-id] kind path:line::name", "index": compact_index(construct_index, 9000), "rule": "Use only constructs present in the index. If a construct cannot be resolved, mark it UNRESOLVED instead of inventing it."},
        "generated_by": "ai-coding-harness.project_profile",
    }


def main() -> int:
    print(json.dumps(build_profile(), indent=2, ensure_ascii=False)); return 0
