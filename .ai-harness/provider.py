#!/usr/bin/env python3
"""Provider bridge with live, provider-native tool/usage/cache telemetry."""
...
from runtime.agent_turn import AgentTurnStateMachine, CACHE_PREFIX, OBSERVATION_PREFIX, USAGE_PREFIX
...
def streaming_command(command: list[str]) -> list[str]:
    """Request machine-readable streaming from supported headless CLIs."""
...
def normalize_tool_event(row: dict[str, Any], sequence: int) -> dict[str, Any] | None:
    """Normalize Claude/Gemini and generic JSONL tool events into harness observations."""
    payload = _event_payload(row)
    ...
    tool = _first(block, "tool_name", "tool", "name", "function_name")
    tool = tool or _first(item, "tool_name", "tool", "name", "function_name")
    tool = tool or _first(payload, "tool_name", "tool", "name", "function_name")
    ...
    return {
        ...
        "tool": str(tool),
        "status": str(status),
        "duration_ms": float(_first(payload, "duration_ms", "duration") or 0),
        "result_digest": _stable_digest(result) if result is not None else "",
...
def normalize_cache(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if isinstance(usage, dict) and any(k in usage for k in ("cache_read_input_tokens", "cache_creation_input_tokens")):
        return {
            "hit": int(usage.get("cache_read_input_tokens", 0) or 0) > 0,
    ...
    return {
        ...
        "read_tokens": int(_first(cache, "read_tokens", "cached_tokens", "cached_content_token_count") or 0),
        "write_tokens": int(_first(cache, "write_tokens", "cache_write_tokens") or 0),
        "cache_key": str(_first(cache, "cache_key", "prompt_cache_key")) if _first(cache, "cache_key", "prompt_cache_key") else None,
        "source": "provider",
    }
...
def main() -> int:
    ...
    try:
        ...
        process = subprocess.Popen(
            command + [prompt], cwd=workspace, env=os.environ.copy(), text=True,
        ...
        with raw_path.open("w", encoding="utf-8") as raw_handle:
            ...
            for line in process.stdout:
                ...
                if tool and tool["status"] in {"started", "running", "in_progress"}:
                    sequence += 1
                    if turn.turn.state == "acting":
                        turn.transition("observing")
                    turn.observe_tools(OBSERVATION_PREFIX + json.dumps({**tool, "status": "started"}))
                    turn.transition("acting")
                elif tool:
                    sequence += 1
                    if turn.turn.state == "acting":
                        turn.transition("observing")
                    turn.observe_tools(OBSERVATION_PREFIX + json.dumps(tool))
                    turn.transition("acting")
    ...
    turn.decide(
        verification_score=1.0 if code == 0 else 0.0,
        evidence_score=1.0 if transcript else 0.0,
        uncertainty=0.0 if code == 0 else 0.6,
        regressions=0 if code == 0 else 1,
    )
