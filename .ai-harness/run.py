...
def invoke(provider: dict[str, Any], prompt_file: Path, phase: str, run_dir: Path, output_file: Path, timeout: int, dry_run: bool) -> tuple[int, str, float]:
    ...
    try:
        ...
        output = f"ERROR: {exc}"
        output_file.write_text(output + "\n", encoding="utf-8")
        return 127, output, time.monotonic() - started
...
def repair_after_failure(config: dict[str, Any], provider: dict[str, Any], run_dir: Path, manifest: dict[str, Any], task: str, source: str, jira: str | None, route: dict[str, Any], history: list[str], dry_run: bool) -> bool:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        code, output, duration = invoke(provider, prompt_file, "repair", run_dir, output_file, timeout, dry_run)
...
def main() -> int:
    ...
    if args.jira_file:
        ...
        if not path.is_file():
            print(f"Jira file not found: {path}", file=sys.stderr)
            return 2
    ...
    if args.resume:
        ...
        if not checkpoint_path.exists() or not manifest_path.exists():
            print("Resume requires checkpoint.json and manifest.json", file=sys.stderr)
            return 2
        ...
        route = normalize_route(route_task(provider, config, task, source, args.jira, memory, repo_map, run_dir, args.dry_run))
    ...
    for index, phase in enumerate(phases):
        ...
        if phase == "validate":
            ...
            if not passed and bool(config.get("execution", {}).get("repair_on_verification_failure", True)):
                if not repair_after_failure(config, provider, run_dir, manifest, task, source, args.jira, route, history, args.dry_run):
        ...
        for attempt in range(max_retries + 1):
            ...
            code, output, duration = invoke(provider, prompt_file, phase, run_dir, output_file, phase_timeout, args.dry_run)
