...
class FeedbackLoop:
    ...
    def observe(self, *, task_id: str, outcome: str, verified: bool, strategy: str, evidence: list[str] | None = None) -> dict:
        event = {"ts": time.time(), "task_id": task_id, "outcome": outcome, "verified": bool(verified), "strategy": strategy, "evidence": list(evidence or [])}
    ...
    def evaluate(self, *, strategy: str) -> dict:
        ...
        return {"strategy": strategy, "observations": n, "success_rate": round(success_rate, 4), "verification_rate": round(verification_rate, 4), "candidate_eligible": eligible, "activation": "blocked_until_regression_and_safety_gates" if eligible else "insufficient_evidence"}

    def candidate(self, *, strategy: str, proposed_change: str) -> dict:
        evaluation = self.evaluate(strategy=strategy)
        return {"candidate_id": hashlib.sha256(f"{strategy}\0{proposed_change}".encode()).hexdigest()[:16], "strategy": strategy, "proposed_change": proposed_change, "evaluation": evaluation, "active": False, "requires": ["deterministic_regression", "safety_gate", "shadow", "canary", "monitoring"]}
