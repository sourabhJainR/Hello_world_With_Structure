...
class LearningController:
    ...
    def observe(self, *, task_id: str, task_class: str, strategy: str, success: bool,
        ...
        self.experience_store.record(Experience(**asdict(observation), metadata=dict(metadata or {})))
    ...
    def observe_turn(self, snapshot: dict[str, Any], *, task_class: str, strategy: str) -> Observation:
        ...
        return self.observe(
            task_id=str(snapshot.get("turn_id", "unknown")), task_class=task_class, strategy=strategy,
    ...
    def candidate_scores(self, task_class: str, *, min_samples: int = 3, min_lower_bound: float = 0.0,
        ...
        incumbent = self.registry.current(task_class)
        incumbent_score = float(incumbent.score) if incumbent else 0.0
    ...
    def learn_candidates(self, observations: Iterable[Observation] | None = None, *, min_samples: int = 3,
        ...
        all_rows = self.experiences()
        candidates: list[PolicyCandidate] = []
        for task_class in sorted({r.task_class for r in all_rows}):
            incumbent = self.registry.current(task_class)
            incumbent_tuple = (incumbent.policy_id, incumbent.strategy, incumbent.score) if incumbent else None
            candidates.extend(learn([r for r in all_rows if r.task_class == task_class], min_samples=min_samples,
                                    incumbent=incumbent_tuple, min_lower_bound=min_lower_bound,
    ...
    def evaluate_and_promote(self, candidate: PolicyCandidate, cases: Iterable[ReplayCase], runner: Callable[[ReplayCase, PolicyCandidate], Any], *,
                             min_pass_rate: float = 1.0, min_verification_rate: float = 1.0,
        ...
        if not shadow.gate_passed:
            self._append("promotion.blocked", {"policy_id": candidate.policy_id, "reason": "shadow-gate", "report": asdict(shadow)})
            return None
        canary = self.canary_candidate(candidate, selected_cases, runner, exposures=exposures,
                                       min_cases_per_stage=min_cases_per_stage,
                                       min_pass_rate=min_pass_rate, min_verification_rate=min_verification_rate)
    ...
    def promote(self, candidate: PolicyCandidate, replay_result: ReplayResult, *, canary_report: EvaluationReport | None = None,
        ...
        current = self.registry.current(candidate.task_class)
        policy = self.registry.add_candidate(Policy(
            candidate.policy_id, selected_version, candidate.task_class, candidate.strategy,
            confidence=float(candidate.confidence), score=float(candidate.score),
            parent_policy_id=current.policy_id if current else "", evidence_hash=candidate.evidence_hash,
        ))
        promoted = self.registry.promote(policy.policy_id, policy.version, now=now)
        self._persist_registry(); self._append("promotion", asdict(promoted)); return promoted

    def monitor(self, health: PolicyHealth, *, now: int | None = None) -> bool:
        rollback = should_rollback(health)
        self._append("monitor", {**asdict(health), "rollback": rollback, "observed_at": int(time.time()) if now is None else int(now)})
        if rollback:
            ...
            if active is not None:
                ...
                self._persist_registry(); self._append("rollback", asdict(retired))
        return rollback
    ...
    def context_plan(self, *, task_class: str, phase: str, risk: str = "medium", uncertainty: str = "medium") -> ContextPlan:
        return plan_context(phase=phase, risk=risk, uncertainty=uncertainty, policy_strategy=self.active_strategy(task_class))

    def _persist_registry(self) -> None:
        self.learn_dir.mkdir(parents=True, exist_ok=True)
        payload = self.registry.export_jsonl(); tmp = self.registry_path.with_suffix(".tmp")
    ...
    def _append(self, event: str, payload: dict[str, Any]) -> None:
        ...
        with self.audit_path.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps({"timestamp": int(time.time()), "event": event, **payload}, ensure_ascii=False, sort_keys=True) + "\n")
...
def _pair(runner: Callable[[ReplayCase, PolicyCandidate], Any], case: ReplayCase, candidate: PolicyCandidate) -> tuple[bool, bool]:
    ...
    if isinstance(raw, dict):
        return bool(raw.get("success")), bool(raw.get("verified"))
