...
try:
    ...
    from .regression_memory import RegressionMemory
...
class LearningController:
    """Own the learning state machine while never granting execution authority."""
    ...
    def observe(self, *, task_id: str, task_class: str, strategy: str, success: bool,
                ...
                regressions: int = 0, cost: float = 0.0, latency_ms: float = 0.0,
                safety_passed: bool = True, evidence_score: float = 1.0,
                environment_fingerprint: str = "", policy_id: str = "",
                transfer_key: str = "", failure_class: str = "", metadata: dict[str, Any] | None = None) -> Observation:
        observation = Observation(
            task_id=str(task_id), task_class=str(task_class), strategy=str(strategy),
            success=bool(success), accepted=bool(accepted), verification_passed=bool(verification_passed),
            retries=max(0, int(retries)), regressions=max(0, int(regressions)), cost=max(0.0, float(cost)),
            latency_ms=max(0.0, float(latency_ms)), safety_passed=bool(safety_passed),
            evidence_score=max(0.0, min(1.0, float(evidence_score))), environment_fingerprint=str(environment_fingerprint),
            policy_id=str(policy_id), transfer_key=str(transfer_key or task_class), failure_class=str(failure_class),
            timestamp=int(time.time()),
        )
        metadata_dict = dict(metadata or {})
        self.experience_store.record(Experience(**asdict(observation), metadata=metadata_dict))
        self._append("observation", {**asdict(observation), "metadata": metadata_dict})
        # Verified failure evidence can be promoted into durable regression knowledge by callers.
        return observation

    def record_regression(self, *, task_family: str, component: str = "", subsystem: str = "",
                          failure_signature: str = "", invariant: str = "", symptom: str = "",
                          reproduction: str = "", fix: str = "", test_pointer: str = "",
                          evidence_pointer: str = "", severity: str = "medium", confidence: float = 0.0,
                          source_kind: str = "observation", source_ref: str = "", verified: bool = False) -> str:
        from .regression_memory import RegressionKnowledge if __package__ else None
        ...
        knowledge_id = self.regression_memory.record(item, verified=verified)
        self._append("regression-knowledge", {"knowledge_id": knowledge_id, "verified": verified, "task_family": task_family, "source_ref": source_ref})
        return knowledge_id
    ...
    def observe_turn(self, snapshot: dict[str, Any], *, task_class: str, strategy: str) -> Observation:
        ...
        return self.observe(task_id=str(snapshot.get("turn_id", "unknown")), task_class=task_class, strategy=strategy,
            success=state == "completed", accepted=state == "completed" and decision.get("action") != "repair",
            verification_passed=verification >= 0.75, retries=failures, regressions=int(snapshot.get("regressions", 0) or 0),
            ...
            safety_passed=bool(snapshot.get("safety_passed", True)), evidence_score=float(snapshot.get("evidence_score", 1.0) or 0),
            environment_fingerprint=str(snapshot.get("environment_fingerprint", "")), policy_id=str(snapshot.get("policy_id", "")),
    ...
    def candidate_scores(self, task_class: str, *, min_samples: int = 3, min_lower_bound: float = 0.0, min_improvement: float = 0.03) -> list[Any]:
        incumbent = self.registry.current(task_class); incumbent_score = float(incumbent.score) if incumbent else 0.0
        return score_candidates(self.experiences(task_class), task_class=task_class,
            incumbent=(incumbent.policy_id, incumbent.strategy, incumbent_score) if incumbent else None,
            min_samples=min_samples, min_lower_bound=min_lower_bound, min_improvement=min_improvement)
    ...
    def learn_candidates(self, observations: Iterable[Observation] | None = None, *, min_samples: int = 3, min_lower_bound: float = 0.0, min_improvement: float = 0.03) -> list[PolicyCandidate]:
        ...
        for row in rows: self.experience_store.record(Experience(**asdict(row), metadata={"source": "controller-input"}))
        all_rows = self.experiences(); candidates: list[PolicyCandidate] = []
        for task_class in sorted({r.task_class for r in all_rows}):
            incumbent = self.registry.current(task_class); incumbent_tuple = (incumbent.policy_id, incumbent.strategy, incumbent.score) if incumbent else None
            candidates.extend(learn([r for r in all_rows if r.task_class == task_class], min_samples=min_samples, incumbent=incumbent_tuple, min_lower_bound=min_lower_bound, min_improvement=min_improvement))
    ...
    def select_regression_cases(self, candidate: PolicyCandidate, cases: Iterable[ReplayCase], *, limit: int = 25) -> SelectedRegressionSet:
        case_list = list(cases); rows = self.experiences(candidate.task_class, 1000)
        historical = [r.task_id for r in rows if r.regressions > 0 or r.failure_class]; recent = [r.task_id for r in rows[:50] if not r.success or r.regressions > 0]
        knowledge = self.regression_memory.retrieve(task_family=candidate.task_class, limit=limit)
        selection = select_regressions(case_list, task_class=candidate.task_class, limit=limit, historical_failures=historical, recent_failures=recent, historical_knowledge=knowledge, seed=candidate.evidence_hash)
    ...
    def promote(self, candidate: PolicyCandidate, replay_result: ReplayResult, *, canary_report: EvaluationReport | None = None, shadow_report: EvaluationReport | None = None, canary_plan: CanaryPlan | None = None, version: int | None = None, now: int | None = None) -> Policy | None:
        ...
        if not replay_result.passed or not shadow_passed or not canary_passed or candidate.risk not in {"low", "medium"} or candidate.confidence < 0.80:
            self._append("promotion.blocked", {"policy_id": candidate.policy_id, "reason": "promotion-gate", "replay_passed": replay_result.passed, "shadow_passed": shadow_passed, "canary_passed": canary_passed, "confidence": candidate.confidence, "risk": candidate.risk}); return None
        selected_version = self.registry.next_version(candidate.task_class) if version is None else int(version); current = self.registry.current(candidate.task_class)
        policy = self.registry.add_candidate(Policy(candidate.policy_id, selected_version, candidate.task_class, candidate.strategy, confidence=float(candidate.confidence), score=float(candidate.score), parent_policy_id=current.policy_id if current else "", evidence_hash=candidate.evidence_hash))
        promoted = self.registry.promote(policy.policy_id, policy.version, now=now); self._persist_registry(); self._append("promotion", asdict(promoted)); return promoted
    ...
    def monitor(self, health: PolicyHealth, *, now: int | None = None) -> bool:
        ...
        if rollback:
            ...
            if active is not None:
                retired = self.registry.rollback(active.policy_id, active.version, now=now, restore_previous=True); self._persist_registry(); self._append("rollback", asdict(retired))
        return rollback
