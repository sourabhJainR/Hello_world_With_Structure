...
def _quality(rows: Sequence[Observation]) -> float:
    if not rows:
        return 0.0
    return sum(
        0.35 * float(r.success) + 0.25 * float(r.accepted) +
        0.25 * float(r.verification_passed) + 0.10 * float(r.regressions == 0) +
        0.05 * float(r.safety_passed)
        for r in rows
    ) / len(rows)
...
def _coerce(row: Observation | Experience) -> Observation:
    ...
    return Observation(
        ...
        success=row.success, accepted=row.accepted, verification_passed=row.verification_passed,
        retries=row.retries, regressions=row.regressions, cost=row.cost, latency_ms=row.latency_ms,
        safety_passed=row.safety_passed, evidence_score=row.evidence_score,
        environment_fingerprint=row.environment_fingerprint, policy_id=row.policy_id,
        transfer_key=row.transfer_key, failure_class=row.failure_class, timestamp=row.timestamp,
    )


def score_candidates(
    observations: Iterable[Observation | Experience], *,
    task_class: str | None = None,
    incumbent: tuple[str, str, float] | None = None,
    min_samples: int = 5,
    min_lower_bound: float = 0.70,
    min_improvement: float = 0.03,
) -> list[CandidateScore]:
    """Rank strategies using outcome quality and a small-sample confidence penalty."""
    rows = [_coerce(x) for x in observations]
    if task_class:
        rows = [x for x in rows if x.task_class == task_class]
    ...
    for (family, strategy), group in groups.items():
        ...
        if len(group) < min_samples:
            reasons.append(f"needs {min_samples - len(group)} more samples")
        if lower < min_lower_bound:
            reasons.append("confidence lower bound below gate")
        if improvement < min_improvement:
            reasons.append("improvement below gate")
        if regression_rate > 0:
            reasons.append("contains regression evidence")
        result.append(CandidateScore(
            policy_id=_id({"task_class": family, "strategy": strategy, "evidence": sorted(r.task_id for r in group)}),
            task_class=family, strategy=strategy, score=round(score, 6), lower_bound=round(lower, 6),
            quality=round(quality, 6), sample_count=len(group), regression_rate=round(regression_rate, 6),
            retry_rate=round(retry_rate, 6), efficiency=round(efficiency, 6),
            improvement_over_incumbent=round(improvement, 6), eligible=not reasons, reasons=tuple(reasons),
        ))
    return sorted(result, key=lambda x: (x.eligible, x.score, x.lower_bound, x.sample_count, x.policy_id), reverse=True)


def learn(
    observations: Iterable[Observation | Experience], min_samples: int = 5, *,
    incumbent: tuple[str, str, float] | None = None,
    min_lower_bound: float = 0.70,
    min_improvement: float = 0.03,
    ...
    rows = [_coerce(x) for x in observations]
    scored = score_candidates(rows, incumbent=incumbent, min_samples=min_samples,
                              min_lower_bound=min_lower_bound, min_improvement=min_improvement)
    ...
    for score in scored:
        ...
        confidence = min(0.99, max(score.lower_bound, 0.5 * score.lower_bound + 0.5 * score.quality))
        risk = "low" if score.regression_rate == 0 and score.lower_bound >= 0.82 else "medium"
        candidates.append(PolicyCandidate(
            policy_id=score.policy_id, task_class=score.task_class, strategy=score.strategy,
            reason=(f"{score.sample_count} observations; quality={score.quality:.0%}, "
                    f"Wilson lower bound={score.lower_bound:.0%}, score={score.score:.3f}."),
            evidence=evidence, confidence=round(confidence, 4), risk=risk, score=score.score,
            lower_bound=score.lower_bound, sample_count=score.sample_count,
            incumbent_policy_id=incumbent[0] if incumbent else "", improvement=score.improvement_over_incumbent,
