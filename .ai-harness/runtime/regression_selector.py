...
def select_regressions(cases: Iterable[ReplayCase], *, task_class: str, limit: int = 25, historical_failures: Iterable[str] = (), recent_failures: Iterable[str] = (), seed: str = "") -> SelectedRegressionSet:
    ...
    for case in sorted(target, key=score, reverse=True):
    ...
    if len(selected) < bounded:
        for case in sorted(neighbors, key=score, reverse=True):
            if case.case_id not in seen: selected.append(case); seen.add(case.case_id)
            if len(selected) >= bounded: break
    coverage = {"same_family": sum(c.task_class == task_class for c in selected), "historical_failures": sum(c.case_id in historical for c in selected), "recent_failures": sum(c.case_id in recent for c in selected), "neighbor_families": sum(c.task_class != task_class for c in selected)}
    rationale = ("same task family is prioritized", "historical and recent failures receive priority", "neighboring families are added only to fill the bounded set", "selection is deterministic for reproducible promotion decisions")
