...
class SandboxPolicy:
    ...
    env_allowlist: tuple[str, ...] = ("PATH", "HOME", "USER", "LANG", "LC_ALL", "TEMP", "TMP", "TMPDIR", "PYTHONPATH", "VIRTUAL_ENV", "CI", "TERM", "SystemRoot", "ComSpec")
    denied_patterns: tuple[str, ...] = (r"(^|/)(rm|rmdir|del)(\s|$)", r"(^|/)(shutdown|reboot)(\s|$)", r"(^|/)mkfs(\.|\s|$)", r"(^|/)dd(\s|$)")
...
def run(command: list[str], *, workspace: Path, policy: SandboxPolicy | None = None) -> subprocess.CompletedProcess[str]:
    ...
    try:
        completed = subprocess.run(command, cwd=workspace, env=_limited_env(policy), shell=False, text=True, capture_output=True, timeout=max(1, int(policy.timeout_seconds)), preexec_fn=_resource_limits(policy) if sys.platform != "win32" else None, check=False)
