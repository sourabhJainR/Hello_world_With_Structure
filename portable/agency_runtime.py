...
class LedgerEntry:
    ...
    def event_id(self) -> str:
        raw = f"{self.event}|{self.detail}|{'|'.join(self.evidence_ids)}"
...
def plan_task(task: TaskProfile, registry: Mapping[str, object] | None = None, support_limit: int = 2) -> list[Assignment]:
    """Select one primary and optional supporting/reviewer specialists deterministically."""
    if support_limit < 0:
...
def evidence_id(item: EvidenceItem) -> str:
    return sha256(f"{item.source}|{item.claim}|{item.locator}".encode("utf-8")).hexdigest()[:16]
...
def execute(
    ...
    """Run one bounded work unit and convert its structured output into a receipt."""
    assignments = plan_task(task, registry=registry)
    result = ExecutionResult(task=task, assignments=assignments)
