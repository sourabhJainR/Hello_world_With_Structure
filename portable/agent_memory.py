...
class AgentMemory:
    ...
    def __init__(self, root: Path, agent: str, *, run_id: str | None = None,
                 max_entries: int = 32, max_chars: int = 12000) -> None:
    ...
    def _snapshot_unlocked(self) -> list[dict[str, Any]]:
        ...
        for line in self.path.read_text(encoding="utf-8").splitlines():
            ...
            if isinstance(row, dict) and (self.run_id is None or row.get("run_id") in {None, self.run_id}):
                rows.append(row)
    ...
    def read(self, limit: int = 5000) -> str:
        ...
        return text if len(text) <= limit else text[:max(200, limit - 50)] + "\n...[agent memory compacted]"

    def remember(self, text: str, *, kind: str = "note") -> None:
        clean = str(text).strip()
        if not clean: return
        row = {"schema_version": 1, "kind": kind, "text": clean[:2000],
               "run_id": self.run_id, "recorded_at": int(time.time())}
        with _lock(self.lock_path):
            rows = self._snapshot_unlocked()
            rows.append(row)
