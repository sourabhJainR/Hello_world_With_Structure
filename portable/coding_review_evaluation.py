...
The local model remains an advisory reviewer. This module scores review findings
against deterministic repository evidence supplied by the caller and maintains
bounded confidence calibration from verified review outcomes.
"""
...
class CodingReviewEvaluator:
    """Evaluate local coding reviews without trusting the model's self-score."""
    ...
    def evaluate(self, observations: Iterable[CodingReviewObservation]) -> CodingReviewCalibration:
        ...
        # A review's useful-confidence target is the independently verified
        # finding validity. This deliberately ignores the model's rationale.
        ...
        return CodingReviewCalibration(
            len(rows), validity, grounding, actionable, regression,
            hallucination, calibration_error, usable,
        )

    def admission(self, calibration: CodingReviewCalibration) -> Mapping[str, object]:
        """Return a bounded policy signal; never grants mutation authority."""
