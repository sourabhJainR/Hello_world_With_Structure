"""Canonical operational capability core for AER.

Hermes-inspired capabilities are implemented here once and composed by AER:
memory/recall, progressive skills, toolsets, background processes, delegation,
scheduling, terminal backend selection and completion-quality checks.
"""
...
class CapabilityRegistry:
    SPECS = {name: CapabilitySpec(name, name.replace("_", " "), builtin=name in {
        "search", "terminal", "file", "skills", "todo", "memory", "session_search", "cronjob", "code_execution", "delegation", "clarify"
...
class MemoryStore:
    """Bounded, deduplicated memory plus exact SQLite FTS5 session recall."""
    ...
    def __init__(self, root: Path | str | None = None, *, memory_limit: int = 2200, user_limit: int = 1375) -> None:
        ...
        self.memory_limit, self.user_limit = memory_limit, user_limit
    ...
    def _scan(text: str) -> None:
        if any(p.search(text) for p in _SENSITIVE) or any(p.search(text) for p in _INJECTION) or any(ord(ch) in _BIDI for ch in text):
    ...
    def add(self, target: str, content: str, *, source: str = "agent", intent_digest: str | None = None, approval_required: bool = False) -> MemoryEntry | MemoryMutation:
        self._scan(content)
        with self._lock:
            items = self.list(target)
            duplicate = next((item for item in items if item.content == content), None)
            if duplicate:
                return duplicate
            used = sum(len(item.content) for item in items)
            if used + len(content) > self._limit(target):
                raise ValueError(f"{target} memory capacity exceeded; consolidate before adding")
    ...
    def index_session(self, session_id: str, messages: Sequence[tuple[str, str]]) -> None:
        with sqlite3.connect(self.db) as conn:
            for seq, (role, content) in enumerate(messages, 1):
                ...
                now = time.time()
                conn.execute("INSERT INTO session_messages VALUES(?,?,?,?,?)", (session_id, seq, role, content, now))
    ...
    def search_session(self, query: str, *, limit: int = 20) -> list[SessionMessage]:
        ...
        with sqlite3.connect(self.db) as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute("SELECT session_id, seq, role, content FROM session_fts WHERE session_fts MATCH ? ORDER BY rank LIMIT ?", (query, limit))
            out = []
            for row in rows:
                stamp = conn.execute("SELECT created_at FROM session_messages WHERE session_id=? AND seq=?", (row["session_id"], row["seq"])).fetchone()[0]
                out.append(SessionMessage(row["session_id"], int(row["seq"]), row["role"], row["content"], float(stamp)))
            return out
    ...
    def _insert(self, target: str, content: str, source: str, intent_digest: str | None) -> MemoryEntry:
        ...
        with sqlite3.connect(self.db) as conn:
            conn.execute("INSERT OR IGNORE INTO memory(entry_id,target,content,created_at,updated_at,source,intent_digest) VALUES(?,?,?,?,?,?,?)", (entry.entry_id, entry.target, entry.content, entry.created_at, entry.updated_at, entry.source, entry.intent_digest))
        return entry
...
class SkillRegistry:
    """Progressive-disclosure procedural knowledge registry."""
    ...
    def scan(self) -> list[dict[str, Any]]:
        ...
        for root in self.roots:
            ...
            for path in sorted(root.rglob("SKILL.md")):
                try:
                    text = path.read_text(encoding="utf-8"); meta, body = self._parse(text); self._scan_security(text)
                    items.append({"path": str(path), "status": "ready", "metadata": meta, "description": meta.get("description", ""), "body_chars": len(body)})
    ...
    def list(self, *, available_tools: Iterable[str] = (), platform: str | None = None) -> list[dict[str, Any]]:
        ...
        for item in self.scan():
            ...
            policy = nested.get("aer", nested.get("hermes", {})) if isinstance(nested, dict) else {}
            required = set(policy.get("requires_tools", ()) or ()) | set(policy.get("requires_toolsets", ()) or ())
            fallbacks = set(policy.get("fallback_for_tools", ()) or ()) | set(policy.get("fallback_for_toolsets", ()) or ())
            if required and not required.issubset(tools): continue
            if fallbacks and fallbacks.intersection(tools): continue
    ...
    def view(self, name: str, *, reference: str | None = None) -> str:
        ...
        target = Path(item["path"]).parent / (reference or "SKILL.md")
        text = target.read_text(encoding="utf-8"); self._scan_security(text); return text

    def learn(self, name: str, description: str, procedure: str, *, root: Path | str | None = None, references: Mapping[str, str] | None = None) -> Path:
        if len(description.strip()) > 60: raise ValueError("skill description must be <= 60 characters")
        self._scan_security(procedure); root_path = Path(root or self.roots[0]).expanduser(); target = root_path / name; target.mkdir(parents=True, exist_ok=True)
        ...
        for rel, content in (references or {}).items():
            self._scan_security(content); p = target / "references" / rel; p.parent.mkdir(parents=True, exist_ok=True); p.write_text(content, encoding="utf-8")
    ...
    def _scan_security(text: str) -> None:
        if any(p.search(text) for p in _SENSITIVE + _INJECTION) or any(ord(ch) in _BIDI for ch in text):
            raise ValueError("skill rejected by security scan")


class ProcessManager:
    """Background command lifecycle with redacted durable completion receipts."""
    def __init__(self, root: Path | str | None = None, *, max_receipts: int = 64, receipt_days: int = 7, output_tail: int = 200_000) -> None:
        self.root = Path(root or Path.home() / ".aer" / "processes").expanduser(); self.root.mkdir(parents=True, exist_ok=True)
        self.max_receipts, self.receipt_days, self.output_tail = max_receipts, receipt_days, output_tail
        self._processes: dict[str, subprocess.Popen[str]] = {}; self._meta: dict[str, dict[str, Any]] = {}; self._lock = threading.RLock()

    def start(self, command: Sequence[str], *, cwd: Path | str | None = None, env: Mapping[str, str] | None = None) -> str:
        sid = hashlib.sha256(f"{time.time_ns()}|{command}".encode()).hexdigest()[:16]
    ...
    def poll(self, session_id: str) -> ProcessReceipt:
        with self._lock:
            ...
            output = (proc.stdout.read() if proc.stdout else "")[-self.output_tail:]
            receipt = ProcessReceipt(session_id, self._meta[session_id]["command"], self._meta[session_id]["started_at"], time.time(), code, self._redact(output), "completed" if code == 0 else "failed")
            self._persist(receipt); self._processes.pop(session_id, None); self._meta.pop(session_id, None); self._prune(); return receipt
...
class DelegationManager:
    """Parallel worker execution governed by TaskPlan dependency status."""
    ...
    def run(self, plan: TaskPlan, worker: Callable[[Task], Any], *, depth: int = 0) -> list[DelegationReceipt]:
        ...
        pending = dict(plan.tasks); done: set[str] = set(); receipts: list[DelegationReceipt] = []
        while pending:
            ready = [task for task in pending.values() if all(plan.tasks[dep].status == "done" or dep in done for dep in task.dependencies)]
            if not ready: raise RuntimeError("delegation plan is blocked or cyclic")
            batch = ready[: self.max_concurrent]
            with ThreadPoolExecutor(max_workers=min(self.max_concurrent, len(batch))) as pool:
                futures = {pool.submit(worker, task): task for task in batch}
                batch_failed: set[str] = set()
                for future in as_completed(futures):
                    task = futures[future]; started = time.time()
                    try:
                        output = future.result(); receipts.append(DelegationReceipt(task.id, "passed", started, time.time(), 1, output)); done.add(task.id); task.status = "done"
                    except Exception as exc:
                        receipts.append(DelegationReceipt(task.id, "failed", started, time.time(), 1, error=f"{type(exc).__name__}: {exc}")); batch_failed.add(task.id); task.status = "blocked"
                if batch_failed:
                    for task in batch: pending.pop(task.id, None)
                    if any(dep in batch_failed for t in pending.values() for dep in t.dependencies):
                        break
            for task in batch: pending.pop(task.id, None)
        return receipts
...
class CronStore:
    ...
    def upsert(self, job: CronJob) -> None:
        with self._lock:
            rows = [x for x in self.list() if x.job_id != job.job_id] + [job]
            self.path.write_text(json.dumps([asdict(x) for x in sorted(rows, key=lambda x: x.job_id)], indent=2) + "\n", encoding="utf-8")
...
class OutputQualityGate:
    ...
    def evaluate(self, report: Mapping[str, Any], *, changed_files: Sequence[str] = ()) -> QualityReport:
        ...
        if isinstance(verification, Mapping) and verification.get("passed") is not True and not verification.get("blocked_reason"):
            findings.append(QualityFinding("error", "verification_missing", "completion requires verification evidence or a blocking reason"))
...
class HermesCapabilityRuntime:
    def __init__(self, *, state_root: Path | str | None = None, skill_roots: Sequence[Path | str] = ()):
        ...
        self.registry = CapabilityRegistry(); self.memory = MemoryStore(root / "memory")
        self.skills = SkillRegistry(skill_roots or (root / "skills",)); self.processes = ProcessManager(root / "processes"); self.cron = CronStore(root / "cron" / "jobs.json")
        self.delegation = DelegationManager(); self.quality = OutputQualityGate()
