...
2. `to-spec` when the work spans sessions or needs a durable build contract.
3. `to-tickets` when the spec decomposes into independently verifiable units with dependencies.
4. `implement` for controlled production changes. `implement` drives `tdd` where a behavioral seam exists and closes with `code-review`.
5. `verify -> review -> artifact -> rollout` using the existing orchestration and provenance contracts.
...
Keep the selected flow bounded. Carry the existing intent, context evidence, artifact, verification, and provenance identifiers through every handoff. Do not introduce a parallel planning, memory, or receipt abstraction.
