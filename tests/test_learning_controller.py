...
class LearningControllerTests(unittest.TestCase):
    def test_promotion_changes_context_strategy_and_rollback_restores_previous(self):
        with tempfile.TemporaryDirectory() as tmp:
            registry = PolicyRegistry([Policy("old", 1, "dotnet_bug", "history_first", "active", .88, promoted_at=1)])
            ...
            result = controller.replay_candidate(candidate, [ReplayCase("r1", "dotnet_bug", True)], lambda _case, _candidate: (True, True))
            promoted = controller.promote(candidate, result, version=2, now=2)
    ...
    def test_failed_replay_is_never_promoted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = controller.replay_candidate(candidate, [ReplayCase("r1", "bug", True), ReplayCase("r2", "bug", True)], lambda case, _candidate: (case.case_id == "r1", True))
