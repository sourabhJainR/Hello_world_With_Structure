...
class P0RuntimeTests(unittest.TestCase):
    def test_state_shape_and_schema_version(self):
        s=new_state('T-1','fix export duplication'); self.assertEqual(validate_state(s),[]); self.assertEqual(s['schema_version'],'1.0')
    ...
    def test_proof_is_deterministic_for_same_state(self):
        s=new_state('T-4','x'); a=proof_bundle(s); b=proof_bundle(s); self.assertEqual(a['proof_id'],b['proof_id'])
